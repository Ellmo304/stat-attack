...
module.exports = function (team, scores) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (results[i].status !== 'FINISHED' && results[i].homeGoals === null && results.length === 1) {
      string += `The match betwen ${formatString(results[i].home)} and ${formatString(results[i].away)} has not started yet.`;
    }
      ...
      string += `${formatString(results[i].home)}: ${results[i].homeGoals === 0 ? 'nil' : results[i].homeGoals}, ${formatString(results[i].away)}: ${results[i].awayGoals === 0 ? 'nil' : results[i].awayGoals}. `;
    }
  ...
  this.emit(':ask', `${string} Can I help with anything else?`, 'Can I help with anything else?');
};
