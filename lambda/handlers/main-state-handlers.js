...
const mainStateHandlers = CreateStateHandler(STATES.MAIN, {
  ...
  'AMAZON.YesIntent': function () {
    ...
    if (this.attributes.expecting === 'continueTable') {
      ...
      this.emit(
        ':ask',
        `In 11th place are ${formatTeam(standings[10].name)} with ${standings[10].points}. They are followed by ${formatTeam(standings[11].name)} with ${standings[11].points}, ${formatTeam(standings[12].name)} with ${standings[12].points} and ${formatTeam(standings[13].name)} with ${standings[13].points}. ${formatTeam(standings[14].name)} are 15th with ${standings[14].points}, followed by ${formatTeam(standings[15].name)} with ${standings[15].points} and ${formatTeam(standings[16].name)} with ${standings[16].points}. Making up the bottom 3 are ${formatTeam(standings[17].name)} with ${standings[17].points}, ${formatTeam(standings[18].name)} with ${standings[18].points} and ${formatTeam(standings[19].name)} with ${standings[19].points}. Can I help with anything else today?`,
